		...
		if (argv[i][0] == '-') {
			...
			case 'h':
				info_message(
					...
					"	-e FILE         Write hardware event log to FILE (or tcp addr:port)\n"
					"	-b FRAMES       Headless mode, exit after FRAMES frames\n"
					"	-t              Force no terminal (disable interactive debugger input)\n"
					"	--dma-log FILE  Log all VDP DMA transfers to FILE\n"
					"	--dma-history N Set DMA history buffer size (min 16, default 512)\n"
				);
				return 0;
			default:
